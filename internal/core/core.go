...
import (
	...
	"gopkg.in/alecthomas/kingpin.v2" // 命令行解析工具

	"github.com/zhangdapeng520/zdpgo_rtsp_server/internal/conf"
	"github.com/zhangdapeng520/zdpgo_rtsp_server/internal/confwatcher"
	"github.com/zhangdapeng520/zdpgo_rtsp_server/internal/externalcmd"
	"github.com/zhangdapeng520/zdpgo_rtsp_server/internal/logger"
	"github.com/zhangdapeng520/zdpgo_rtsp_server/internal/rlimit"
)
...
func New(args []string) (*Core, bool) {
	// 创建kingpin对象
	k := kingpin.New("rtsp-simple-server", "rtsp服务 ")
	argVersion := k.Flag("version", "是否打印版本").Bool()                                         // 版本参数
	argConfPath := k.Arg("confpath", "配置文件路径，默认：config.yml.").Default("config.yml").String() // 配置文件
	kingpin.MustParse(k.Parse(args))                                                         // 解析参数

	if *argVersion { // 如果打印版本
	...
	p := &Core{
		ctx:          ctx,                   // 上下文对象
		ctxCancel:    ctxCancel,             // 取消上下文对象
		confPath:     *argConfPath,          // 参数路径
		apiConfigSet: make(chan *conf.Conf), // 配置管道
		done:         make(chan struct{}),   // 执行管道
	}
