...
release-nodocker:
	...
	$(eval GOBUILD := go build -ldflags '-X github.com/zhangdapeng520/zdpgo_rtsp_server/internal/core.version=$(VERSION)')
	rm -rf tmp && mkdir tmp
...
RUN export CGO_ENABLED=0 $${OPTS} \
	&& go build -ldflags "-X github.com/zhangdapeng520/zdpgo_rtsp_server/internal/core.version=$$VERSION" -o /rtsp-simple-server
